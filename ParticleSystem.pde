...
public class ParticleSystem {
  ...
  PShader unlitShader = loadShader("unlit_frag.glsl", "unlit_vert.glsl");
  ...
  public void generateNewParticles(float dt) {
    ...
    for(int i = 0; i < stochasticNewParticles; i++) {
      particleCoords.add(new Vec3(emitterPosition.plus(new Vec3(random(-0.5,0.5),random(-0.5,0.5),random(-0.5,0.5)))));
  ...
  public void updateAll(float dt, boolean withCollisions) {
    generateNewParticles(dt);
    updateParticlePositions(dt);
    updateParticleProperties(dt);
    removeDeadParticles();
    if (withCollisions) {
      checkParticlesForCollisions(rigidBodies);
    }
  }
